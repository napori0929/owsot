...
function updateHUD(data) {
  ...
  if (teamKey !== lastTeamKey) {
    ...
    if (teamLogoBig) teamLogoBig.src = t1.logo || "";
    if (teamNameBig) teamNameBig.textContent = t1.fullName || t1.name || "";
    if (teamHeader) teamHeader.style.setProperty("--team-color", t1.color || "#00c36a");
  }

  // 로스터는 rosterKey 바뀔 때만 다시 그려서 영상 리셋 최소화
  if (rosterKey !== lastRosterKey) {
    lastRosterKey = rosterKey;
    renderTeamRoster("team1RosterCards", roster);
  }
}
